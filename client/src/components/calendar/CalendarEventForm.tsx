...
interface CalendarEventFormProps {
  ...
  onDelete?: (data: { event: CalendarEvent, excludeRecurringEvent: boolean }) => void
}

const _CalendarEventForm: React.FC<CalendarEventFormProps & FormComponentProps> = props => {
  ...
  function getRecursOnArray(): CalendarEvent['recursOn'] {
    const selctedDays: number[] | undefined = getFieldValue('recursOn')
    let recursOn: CalendarEvent['recursOn'] = [false, false, false, false, false, false, false]
    if (!selctedDays) return undefined
    selctedDays.map((dayIndex) => recursOn![dayIndex] = true)
    return recursOn
  }

  function getEventFromFields(): CalendarEvent {
...
function recurrsArrayToNumArray(recursOn: CalendarEvent['recursOn']): Array<number> {
  if (recursOn) {
...
export const CalendarEventForm = Form.create<FormComponentProps & CalendarEventFormProps>({
